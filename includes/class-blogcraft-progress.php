...
class Blogcraft_Progress {
	...
	public static function steps() {
		return array(
			'research'  => __( 'Reading what is already out there', 'dicecodes-ai-blog-writer' ),
			'outline'   => __( 'Planning the shape of the post', 'dicecodes-ai-blog-writer' ),
			'draft'     => __( 'Writing the opening', 'dicecodes-ai-blog-writer' ),
			'section'   => __( 'Writing each section', 'dicecodes-ai-blog-writer' ),
			'faq'       => __( 'Answering the questions readers ask', 'dicecodes-ai-blog-writer' ),
			'extras'    => __( 'Adding the extra sections', 'dicecodes-ai-blog-writer' ),
			'critique'  => __( 'Reading its own draft back critically', 'dicecodes-ai-blog-writer' ),
			'revise'    => __( 'Rewriting what it found wrong', 'dicecodes-ai-blog-writer' ),
			'verify'    => __( 'Checking links and scoring the result', 'dicecodes-ai-blog-writer' ),
			'publish'   => __( 'Creating the post', 'dicecodes-ai-blog-writer' ),
			'pictures'  => __( 'Finding the pictures', 'dicecodes-ai-blog-writer' ),
			'finishing' => __( 'Linking it up and telling the crawlers', 'dicecodes-ai-blog-writer' ),
		);
	}
	...
	public static function register_menu() {
		add_submenu_page(
			'',
			__( 'Writing a post', 'dicecodes-ai-blog-writer' ),
			__( 'Writing a post', 'dicecodes-ai-blog-writer' ),
	...
	public static function assets( $hook ) {
		...
		wp_localize_script(
			...
			array(
				...
				'stepOf'    => __( 'Step %1$d of %2$d', 'dicecodes-ai-blog-writer' ),
				/* translators: %s: a duration such as "40s" or "2m 10s". */
				'elapsed'   => __( '%s elapsed', 'dicecodes-ai-blog-writer' ),
				/* translators: %s: a duration such as "40s" or "2m 10s". */
	...
	public static function render() {
		...
		if ( null === $job ) {
			echo '<h1>' . esc_html__( 'Writing a post', 'dicecodes-ai-blog-writer' ) . '</h1>';
			echo '<p>' . esc_html__( 'There is no post here. Start one from Write a post.', 'dicecodes-ai-blog-writer' ) . '</p>';
			printf(
		...
		if ( 'nothing' === $outcome ) {
			printf(
				...
				esc_html__( 'Nothing left that another writing pass can reach, so no request was made and you were not charged for one. What is still failing is about your site rather than these words.', 'dicecodes-ai-blog-writer' )
			);
		}

		$topic = isset( $job->payload['topic'] ) ? (string) $job->payload['topic'] : '';

		echo '<h1>' . esc_html( '' === $topic ? __( 'Writing a post', 'dicecodes-ai-blog-writer' ) : $topic ) . '</h1>';
	...
	private static function render_steps( $job ) {
		...
		echo '<h2>' . esc_html__( 'What it is doing', 'dicecodes-ai-blog-writer' ) . '</h2>';
		...
		printf(
			...
			esc_html(
				sprintf(
					...
					__( 'Step %1$d of %2$d', 'dicecodes-ai-blog-writer' ),
		...
		printf(
			...
			esc_html(
				$held
					? __( 'Finished writing. Nothing has been added to your site yet.', 'dicecodes-ai-blog-writer' )
					: __( 'This runs while the page is open. Leaving is safe — it picks up where it stopped.', 'dicecodes-ai-blog-writer' )
			)
		);
		echo '</header>';
		...
		printf(
			...
			. '</div>',
			$held ? ' hidden' : '',
			esc_html__( 'The title and the headings appear here as they are decided.', 'dicecodes-ai-blog-writer' )
		);
	...
	private static function render_prospects( $job, $article, $checks ) {
		...
		echo '<h2>' . esc_html__( 'What would hold this back', 'dicecodes-ai-blog-writer' ) . '</h2>';
		...
		if ( empty( $blockers ) ) {
			printf(
				...
				esc_html__( 'Nothing this plugin can see. It does not duplicate a post you already have, it says something that is yours, it is the length it was written to be, and your site points at it.', 'dicecodes-ai-blog-writer' )
			);
			echo '</section>';
	...
	private static function render_score( $score, $checks, $before = 0 ) {
		...
		printf(
			...
			(int) $score,
			esc_html__( 'out of 100', 'dicecodes-ai-blog-writer' )
		);

		echo '<div class="bc-score-words">';
		echo '<h2>' . esc_html__( 'How it scored', 'dicecodes-ai-blog-writer' ) . '</h2>';
		printf(
			'<p>%s</p>',
			esc_html(
				$clears
					? sprintf(
						...
						__( 'Passed %1$d of %2$d checks, and clears your bar of %3$d.', 'dicecodes-ai-blog-writer' ),
						count( $passed ),
					...
					: sprintf(
						/* translators: 1: number of checks passed. 2: checks in total. 3: the threshold set in settings. */
						__( 'Passed %1$d of %2$d checks, which is under your bar of %3$d. You can still create it — it lands as a draft.', 'dicecodes-ai-blog-writer' ),
						count( $passed ),
						count( $passed ) + count( $failed ),
						$threshold
					)
		...
		if ( $before > 0 ) {
			...
			if ( 0 === $gap ) {
				$line = sprintf(
					...
					__( 'The second pass left it at %d. Nothing measurable changed.', 'dicecodes-ai-blog-writer' ),
					(int) $score
				);
			} elseif ( $gap > 0 ) {
				$line = sprintf(
					/* translators: 1: previous score. 2: how many points it gained. */
					__( 'Up from %1$d after another pass, so that one bought %2$d points.', 'dicecodes-ai-blog-writer' ),
					(int) $before,
					(int) $gap
				);
			} else {
				$line = sprintf(
					...
					__( 'Down from %d. A rewrite is not guaranteed to improve anything, and this one did not.', 'dicecodes-ai-blog-writer' ),
					(int) $before
				);
			}
		...
		if ( empty( $failed ) && empty( $passed ) ) {
			echo '<p>' . esc_html__( 'No checks were recorded for this draft.', 'dicecodes-ai-blog-writer' ) . '</p></section>';
		...
		// sixteen that do not.
		if ( ! empty( $failed ) ) {
			echo '<h3 class="bc-check-heading">' . esc_html__( 'Worth a look', 'dicecodes-ai-blog-writer' ) . '</h3>';
			...
			printf(
				...
				esc_html(
					sprintf(
						...
						_n( '%d check passed', '%d checks passed', count( $passed ), 'dicecodes-ai-blog-writer' ),
						count( $passed )
					)
				)
			);
	...
	private static function render_undo() {
		...
		printf(
			...
			esc_html__( 'Put the earlier version back', 'dicecodes-ai-blog-writer' )
		);
		echo '</form>';
	}
	...
	private static function render_improve( $failed ) {
		...
		if ( ! empty( $fixable ) ) {
			...
			printf(
				...
				esc_html__( 'Have another go at these', 'dicecodes-ai-blog-writer' )
			);
			...
			printf(
				...
				esc_html(
					sprintf(
						...
						__( 'Rewriting will not change %s. That one is about your site rather than these words.', 'dicecodes-ai-blog-writer' ),
						implode( ', ', $names )
					)
				)
			);
			...
			printf(
				...
				esc_html(
					sprintf(
						...
						__( '%1$s — wanted %2$s', 'dicecodes-ai-blog-writer' ),
	...
	private static function render_preview( $article, $outline ) {
		...
		echo '<h2>' . esc_html__( 'The draft', 'dicecodes-ai-blog-writer' ) . '</h2>';
		echo '<p>' . esc_html__( 'Edit anything here before it becomes a post. Use Add Media to put a picture between the paragraphs. Nothing is on your site until you create it.', 'dicecodes-ai-blog-writer' ) . '</p>';
		echo '</header>';
		...
		printf(
			...
			esc_html__( 'Title', 'dicecodes-ai-blog-writer' ),
		...
		printf(
			...
			esc_html__( 'Search description', 'dicecodes-ai-blog-writer' ),
			esc_textarea( isset( $outline['meta_description'] ) ? (string) $outline['meta_description'] : '' ),
			esc_html__( 'What shows under the title in search results.', 'dicecodes-ai-blog-writer' )
		);
	...
	private static function render_decision( $job, $score ) {
		...
		echo '<h2>' . esc_html__( 'What happens to it', 'dicecodes-ai-blog-writer' ) . '</h2>';
		printf(
			'<p>%s</p>',
			esc_html(
				$score < $threshold
					? __( 'This scored below your bar. Creating it anyway is fine — it lands as a draft you can edit.', 'dicecodes-ai-blog-writer' )
					: __( 'Creating it adds it to your posts, using the editor your site is set up for.', 'dicecodes-ai-blog-writer' )
			)
		);
		echo '</header>';
		...
		submit_button( __( 'Create the post', 'dicecodes-ai-blog-writer' ), 'primary', 'submit', false );
		echo ' ';
		printf(
			'<a class="button" href="%1$s">%2$s</a>',
			esc_url( admin_url( 'admin.php?page=blogcraft-write' ) ),
			esc_html__( 'Leave it for now', 'dicecodes-ai-blog-writer' )
		);
		echo '</form></section>';
	}
	...
	private static function render_waiting( $job, $resumes ) {
		...
		echo '<h2>' . esc_html__( 'Paused by your provider', 'dicecodes-ai-blog-writer' ) . '</h2>';
		printf(
			'<p>%s</p>',
			esc_html(
				sprintf(
					/* translators: %s: a clock time, such as "3:45 pm". */
					__( 'Your provider is rate limiting, so this is waiting rather than failing. It carries on by itself at about %s, and everything written so far is kept.', 'dicecodes-ai-blog-writer' ),
					$resumes
				)
			)
		);
		...
		printf(
			...
			esc_html__( 'You can close this page. Come back to it from', 'dicecodes-ai-blog-writer' ),
			esc_url( admin_url( 'admin.php?page=' . Blogcraft_Library::PAGE_SLUG ) ),
			esc_html__( 'Written by AI', 'dicecodes-ai-blog-writer' )
		);
	...
	private static function render_failure( $job ) {
		...
		echo '<h2>' . esc_html__( 'It stopped', 'dicecodes-ai-blog-writer' ) . '</h2>';
		echo '</header>';
		...
		printf(
			...
			esc_html__( 'See the full log', 'dicecodes-ai-blog-writer' )
		);
		echo '</section>';
	}
}
