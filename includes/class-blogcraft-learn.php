...
class Blogcraft_Learn {
	...
	public static function style_rules( $seen ) {
		...
		if ( $seen['sentence_words'] > 0 && $seen['sentence_words'] <= 16 ) {
			$rules[] = __( 'Keep sentences short. Most of ours run under sixteen words.', 'blogcraft' );
		} elseif ( $seen['sentence_words'] >= 24 ) {
			$rules[] = __( 'Long sentences are fine here. Do not chop everything into fragments.', 'blogcraft' );
		}

		if ( $seen['para_sentences'] > 0 && $seen['para_sentences'] <= 3 ) {
			$rules[] = __( 'Short paragraphs, two or three sentences each.', 'blogcraft' );
		}

		// Under one per post across the sample is a writer who avoids them.
		if ( $seen['em_dash'] < $seen['posts'] ) {
			$rules[] = __( 'No em dashes.', 'blogcraft' );
		}

		if ( $seen['contractions'] >= ( $seen['posts'] * 5 ) ) {
			$rules[] = __( 'Use contractions. Write the way people speak.', 'blogcraft' );
		} elseif ( 0 === $seen['contractions'] ) {
			$rules[] = __( 'No contractions.', 'blogcraft' );
		}

		if ( 'first' === $seen['person'] ) {
			$rules[] = __( 'Write in the first person. Say what we did and what we found.', 'blogcraft' );
		} elseif ( 'second' === $seen['person'] ) {
			$rules[] = __( 'Address the reader directly as "you".', 'blogcraft' );
		}
	...
	public static function suggest() {
		...
		if ( empty( $seen['posts'] ) ) {
			return array(
				'found'  => 0,
				'fields' => array(),
				'notes'  => array( __( 'There are no posts here yet that Blogcraft did not write, so there is nothing to learn from.', 'blogcraft' ) ),
			);
		}
		...
		if ( 'first' === $seen['person'] ) {
			$fields['voice_point_of_view'] = __( 'First person plural', 'blogcraft' );
		} elseif ( 'second' === $seen['person'] ) {
			$fields['voice_point_of_view'] = __( 'Second person, addressing the reader', 'blogcraft' );
		}

		$notes = array(
			sprintf(
				/* translators: 1: number of posts read. 2: average word count. */
				__( 'Read %1$d of your posts. They average %2$d words.', 'blogcraft' ),
				(int) $seen['posts'],
				(int) $seen['words']
			),
		);
		...
		if ( '' === $written['niche'] ) {
			$notes[] = __( 'The written descriptions need a working AI provider, so only the measured style rules were filled in.', 'blogcraft' );
		}

		$notes[] = __( 'Nothing has been saved. Correct anything that is wrong, then save.', 'blogcraft' );
