...
class Blogcraft_Verify {
	...
	public static function score( $article ) {
		...
		if ( empty( $article['intro'] ) ) {
			$score    -= 15;
			$reasons[] = __( 'No opening paragraph answering the title.', 'dicecodes-ai-blog-writer' );
		}
		...
		if ( count( $sections ) < 3 ) {
			$score    -= 20;
			$reasons[] = __( 'Fewer than three sections.', 'dicecodes-ai-blog-writer' );
		}
		...
		if ( $words < 300 ) {
			$score    -= 25;
			$reasons[] = __( 'Under 300 words. Thin content is the most penalised kind.', 'dicecodes-ai-blog-writer' );
		} elseif ( $words < 600 ) {
			$score    -= 10;
			$reasons[] = __( 'Under 600 words.', 'dicecodes-ai-blog-writer' );
		}
		...
		if ( ! empty( $blueprint['faq'] ) && empty( $article['faq'] ) ) {
			$score    -= 5;
			$reasons[] = __( 'No FAQ section.', 'dicecodes-ai-blog-writer' );
		}

		if ( ! empty( $blueprint['takeaways'] ) && empty( $article['key_takeaways'] ) ) {
			$score    -= 5;
			$reasons[] = __( 'No key takeaways.', 'dicecodes-ai-blog-writer' );
		}
		...
		foreach ( Blogcraft_Voice::banned_words() as $banned ) {
			...
			if ( Blogcraft_Metrics::phrase_count( $haystack, $banned ) > 0 ) {
				...
				$reasons[] = sprintf( __( 'Contains a banned phrase: "%s".', 'dicecodes-ai-blog-writer' ), $banned );
				break;
			}
		}
		...
		if ( count( $headings ) !== count( array_unique( $headings ) ) ) {
			$score    -= 10;
			$reasons[] = __( 'Two sections share a heading.', 'dicecodes-ai-blog-writer' );
		}
