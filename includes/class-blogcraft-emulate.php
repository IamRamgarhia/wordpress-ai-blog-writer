...
class Blogcraft_Emulate {
	...
	public static function fetch( $url ) {
		...
		if ( '' === $url || ! wp_http_validate_url( $url ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'That does not look like a web address.', 'dicecodes-ai-blog-writer' ),
				'title' => '',
				'html'  => '',
			);
		}
		...
		if ( is_wp_error( $response ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'That page could not be read.', 'dicecodes-ai-blog-writer' ),
				'title' => '',
				'html'  => '',
			);
		}
		...
		if ( 200 !== (int) wp_remote_retrieve_response_code( $response ) ) {
			return array(
				'ok'    => false,
				'error' => __( 'That page did not answer with an article.', 'dicecodes-ai-blog-writer' ),
				'title' => '',
				'html'  => '',
			);
		}
	...
	public static function study( $url ) {
		...
		if ( $seen['words'] < 200 ) {
			return array(
				'ok'     => false,
				'error'  => __( 'There was not enough writing on that page to measure. Link to the article itself rather than a category or a home page.', 'dicecodes-ai-blog-writer' ),
				'title'  => $page['title'],
				'notes'  => array(),
				'fields' => array(),
			);
	...
	public static function notes( $seen ) {
		$notes = array(
			sprintf(
				...
				__( '%1$s words across %2$d sections, averaging %3$d words a sentence.', 'dicecodes-ai-blog-writer' ),
			...
			sprintf(
				...
				__( '%1$d links out, %2$d specific figures, %3$d images.', 'dicecodes-ai-blog-writer' ),
		...
		if ( 'first' === $seen['person'] ) {
			$notes[] = __( 'Written in the first person.', 'dicecodes-ai-blog-writer' );
		} elseif ( 'second' === $seen['person'] ) {
			$notes[] = __( 'Addresses the reader as "you".', 'dicecodes-ai-blog-writer' );
		}

		if ( ! empty( $seen['faq'] ) ) {
			$notes[] = __( 'Ends with a run of questions.', 'dicecodes-ai-blog-writer' );
		}

		$notes[] = __( 'Structure only. None of the wording was copied, kept, or shown to a model.', 'dicecodes-ai-blog-writer' );
