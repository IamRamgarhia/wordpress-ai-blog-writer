...
class Blogcraft_Welcome {
	...
	public static function register_menu() {
		add_submenu_page(
			'',
			__( 'Welcome to Blogcraft', 'blogcraft' ),
			__( 'Welcome to Blogcraft', 'blogcraft' ),
	...
	public static function render() {
		...
		echo '<h1>' . esc_html__( 'Three things worth two minutes', 'blogcraft' ) . '</h1>';
		echo '<p class="bc-welcome-lead">' . esc_html__( 'Almost everything that separates a post worth reading from filler is decided before any writing starts. These are the three that matter most. You can skip any of them and change all of them later.', 'blogcraft' ) . '</p>';
	...
	private static function render_rail( $current ) {
		$steps = array(
			...
			'voice'    => __( 'Say who you write for', 'blogcraft' ),
			'research' => __( 'Choose what it may read', 'blogcraft' ),
			'ready'    => __( 'Write something', 'blogcraft' ),
		);
	...
	private static function skip_link() {
		printf(
			...
			esc_html__( 'Skip this and go to the Write screen', 'blogcraft' )
		);
	}
	...
	private static function step_provider() {
		...
		echo '<h2>' . esc_html__( 'Connect a provider', 'blogcraft' ) . '</h2>';
		echo '<p>' . esc_html__( 'Blogcraft has no AI of its own. It uses an account you own, with a key you paste in, and every request is billed to you by that provider rather than passing through anybody else.', 'blogcraft' ) . '</p>';
		echo '<p>' . esc_html__( 'If you have no account anywhere: Google and Groq both have free tiers big enough to write with, and Ollama runs a model on your own machine for nothing at all.', 'blogcraft' ) . '</p>';
	...
	private static function step_voice() {
		...
		echo '<h2>' . esc_html__( 'Say who you write for', 'blogcraft' ) . '</h2>';
		echo '<p>' . esc_html__( 'This is sent with every request afterwards, and it is the single biggest reason two blogs using the same model do not read the same. Two sentences is enough.', 'blogcraft' ) . '</p>';
		...
		printf(
			...
			esc_html__( 'What this blog is about', 'blogcraft' ),
			esc_attr__( 'Home coffee equipment, tested properly rather than unboxed.', 'blogcraft' ),
		...
		printf(
			...
			esc_html__( 'Who is reading, and what they already know', 'blogcraft' ),
			esc_attr__( 'People buying their first proper machine, who know what espresso is but not what a pressure profile is.', 'blogcraft' ),
			esc_textarea( (string) Blogcraft_Settings::get( 'voice_audience' ) )
		);

		printf(
			'<p class="description">%1$s <a href="%2$s">%3$s</a></p>',
			esc_html__( 'Already have posts published?', 'blogcraft' ),
			esc_url( admin_url( 'admin.php?page=blogcraft-settings#bc-card-voice' ) ),
			esc_html__( 'Blogcraft can read them and fill this in from how you actually write.', 'blogcraft' )
		);

		submit_button( __( 'Save and continue', 'blogcraft' ), 'primary', 'submit', false );
		self::skip_link();
		echo '</form>';
	}
	...
	private static function step_research() {
		...
		echo '<h2>' . esc_html__( 'Choose what it may read', 'blogcraft' ) . '</h2>';
		echo '<p>' . esc_html__( 'With research on, the model is handed current sources and writes from them, and the finished draft is checked against those same sources for whether it says anything they do not. With everything off it writes from memory, which dates badly and can cite nothing.', 'blogcraft' ) . '</p>';
		echo '<p>' . esc_html__( 'Nothing is contacted until you tick something here. Both of these are free and need no account.', 'blogcraft' ) . '</p>';
		...
		printf(
			'<p class="description">%1$s <a href="%2$s">%3$s</a></p>',
			esc_html__( 'Paid search providers return more current results.', 'blogcraft' ),
			esc_url( admin_url( 'admin.php?page=blogcraft-settings#bc-card-research' ) ),
			esc_html__( 'They are in Settings, and entirely optional.', 'blogcraft' )
		);
	...
	private static function step_ready() {
		...
		echo '<h2>' . esc_html__( 'One thing before you write', 'blogcraft' ) . '</h2>';
		echo '<p>' . esc_html__( 'On the Write screen there is a field asking what you know that nobody else does. It is the heaviest check on the finished post and the only part of a page a model genuinely cannot produce.', 'blogcraft' ) . '</p>';
